...
export default function AdminDashboard() {
  ...
  // --- RECEIPT LOGIC (Updated to show extras) ---
      ...
      <html>
        <head>
          ...
          <style>
            ...
            .item { margin-bottom: 8px; border-bottom: 1px solid #eee; padding-bottom: 4px; }
            .item-top { display: flex; justify-content: space-between; font-weight: bold; }
            .details { font-size: 10px; color: #555; margin-left: 10px; }
            .total { border-top: 1px dashed #000; margin-top: 10px; padding-top: 10px; font-weight: bold; text-align: right; font-size: 14px; }
        ...
        <body>
          ...
          <div style="font-size: 12px; margin-bottom: 10px;">
            ...
            <p><strong>Type:</strong> ${order.method?.toUpperCase()}</p>
            ${order.method === 'delivery' ? `<p><strong>Addr:</strong> ${order.customer?.address}</p>` : ''}
          </div>
          <hr/>
          ${order.items.map(item => `
            <div class="item">
              ...
              <div class="details">
                ...
                ${item.selectedExtras && item.selectedExtras.length > 0 ? `+ ${item.selectedExtras.join(', ')}` : ''}
              </div>
            </div>
          `).join('')}
          <div class="total">
            ...
            <div key={order.id} className="bg-white/5 border border-white/10 p-4 md:p-6 rounded-xl flex flex-col xl:flex-row gap-6 justify-between items-start">
              ...
              <div className="min-w-[250px] space-y-1">
                ...
                <div className="bg-black/30 p-3 rounded mt-2 text-sm border border-white/5">
                  ...
                  {order.customer?.instructions && <p className="mt-2 text-orange-400 text-xs">" {order.customer.instructions} "</p>}
                </div>
              </div>

              {/* ORDER ITEMS (UPDATED TO SHOW DETAILS) */}
...
// ... (KEEP THE AddItemForm FUNCTION FROM THE PREVIOUS MESSAGE EXACTLY AS IT WAS)
// I am omitting AddItemForm here to save space, but you MUST keep it in the file.
// If you need me to paste the FULL file again with AddItemForm included, just ask.
// (For the user: Copy AddItemForm from the previous successful response and paste it below AdminDashboard)
// -----------------------------------------------------------------------------------------
// RE-PASTING ADDITEMFORM BELOW FOR COMPLETENESS SO YOU CAN COPY-PASTE THE WHOLE FILE:
